...
new_pattern = r'''<!-- Clinic Events for Selected Date -->
...
old_pattern = '''<!-- Clinic Events for Selected Date -->
                                    <template x-for="event in filteredClinicEvents" :key="event.id">
...
# Replace using regex
content = re.sub(new_pattern, old_pattern, content, flags=re.DOTALL)

# Write back
with open(r'c:\xampp\htdocs\iClini V.2\pages\staff\Staff-Appointments.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("✅ Reverted to original design!")
